...
public class Sauron {
    ...
    public static String checker(String input) {
        ...
        Conan.ConanResult conanRes = Conan.automatic_detective(input);
        if (conanRes.isEncoded) {
            System.out.println("Input sospetto rilevato: \"" + input + "\". TIPO: " + conanRes.format + ". Applicazione della sanitizzazione ricorsiva.");
            return conanRes.decoded;
        } else {
    ...
    public static String sanitize(String input, boolean isEmail) {
        ...
        // Include RAD constraints and Sauron script constraints
        // RAD: @ , ‘ , ’ , / , ; , - , “ , ” , #
        // Sauron: < > ( ) " '
    ...
    public static ProcessResult processField(String input, boolean isEmail, boolean required, int minLength, String fieldName) {
        ...
        String conanValue = checker(rawValue);
        String safeValue = sanitize(conanValue, isEmail);
        
        boolean odinResult = Odino.odino_manager(safeValue);
        
        if (!odinResult) {
            result.isValid = false;
            result.errors.add("Input bloccato da Odino: \"" + safeValue + "\"");
            safeValue = null;
        } else {
            System.out.println("Input passato attraverso Odino: \"" + safeValue + "\". Risultato: Confermato.");
        }
